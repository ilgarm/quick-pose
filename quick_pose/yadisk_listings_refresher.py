...
@click.option('--categories', required=True,
              type=click.Choice(('Figure',), case_sensitive=True), multiple=True,
...
def refresh(yadisk_source_path: Path, yadisk_dest_path: Path, categories: [str], mime_types: [str], upload: bool,
            yandex_client_id: str, yandex_client_secret: str, yandex_access_token: str, max_tasks: int):
    if max_tasks <= 0:
